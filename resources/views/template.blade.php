...
.custom-card {
    background-color:  rgba(77 123 161 / 23%) !important; /* 0.5 = 50% transparan */
}
...
:root {
    ...
    --neo-shadow: 6px 6px 0px #334155;
    --neo-shadow-sm: 4px 4px 0px #334155;
    --neo-shadow-lg: 8px 8px 0px #334155;
    --sidebar-width: 280px;
    --bg-primary: #212529;
...
.btn-neo:hover, .neo-btn:hover {
    transform: translate(2px, 2px) !important;
    box-shadow: 2px 2px 0 var(--accent-blue-dark) !important;
    background: var(--bg-tertiary);
}

.modal-neo .modal-content, .neo-card:hover {
...
@media (min-width: 992px) {
    ...
    .close-sidebar-btn, .hamburger-btn, .sidebar-backdrop {
        display: none !important;
    }
...
.modal-neo .modal-dialog {
    margin: 2rem auto;
}

.modal-neo .modal-content {
    border: var(--neo-border) !important;
...
.btn-neo, .modal-neo .btn-close {
    border: var(--neo-border) !important;
}

.modal-neo .modal-header {
    background-color: var(--bg-tertiary) !important;
    border-bottom: var(--neo-border) !important;
    padding: 1.5rem 2rem;
}

.modal-neo .modal-title {
    font-size: 1.5rem;
    color: var(--text-primary);
}
...
.btn-neo, .modal-neo .form-control, .modal-neo .form-control:focus {
    box-shadow: var(--neo-shadow-sm) !important;
}

.modal-neo .btn-close::before {
    content: "✕";
    font-size: 1rem;
}

.modal-neo .modal-body {
    padding: 2rem;
    font-weight: 500;
    line-height: 1.6;
...
.modal-neo .modal-footer {
    border-top: var(--neo-border) !important;
    padding: 1.5rem 2rem;
    background-color: var(--bg-tertiary);
}
...
.demo-title, .modal-neo .form-label {
    color: var(--text-primary);
    text-transform: uppercase;
    letter-spacing: .05em;
}
...
.modal-neo .modal-backdrop {
    background-color: rgba(0, 0, 0, .8);
}

.modal-neo .form-control {
    border: var(--neo-border) !important;
    border-radius: 0 !important;
    font-weight: 500;
    padding: .75rem 1rem;
...
.modal-neo .form-control:focus {
    border-color: var(--accent-blue) !important;
    background-color: var(--bg-secondary) !important;
}

.modal-neo .form-label {
    font-weight: 700;
    font-size: .875rem;
    margin-bottom: .5rem;
    color: var(--text-secondary);
...
<body>
    ...
    <div class="main-container">
        ...
        <aside class="sidebar d-flex flex-column" id="mobileSidebar">
            ...
            <div class="flex-grow-1 overflow-auto p-3">
                ...
                @if(auth()->user()->type == 'admin')
                   ...
                   <!-- Permission & User Auth Management -->
                    <div class="mb-2">
                        <button class="sidebar-item p-3 fw-medium d-flex align-items-center {{ request()->routeIs('admin.users.categories.*', 'admin.users.userauth.*') ? '' : 'collapsed' }} dropdown-toggle"
                                data-bs-toggle="collapse" data-bs-target="#permissionDropdown"
                                aria-expanded="{{ request()->routeIs('admin.users.categories.*', 'admin.users.userauth.*') ? 'true' : 'false' }}">
                            <i class="fas fa-user-cog me-2"></i> Manajemen Akses
                        </button>
                        <div class="collapse sidebar-dropdown {{ request()->routeIs('admin.users.categories.*', 'admin.users.userauth.*') ? 'show' : '' }}" id="permissionDropdown">
                            <!-- Permission Barang -->
            ...
            <!-- Sidebar Footer -->
            <div class="p-3 border-top border-black border-3">
                <div class="d-flex align-items-center">
                    <img src="https://i.pravatar.cc/40" class="rounded-circle border-2 border-black me-2" alt="{{ ucfirst(auth()->user()->type ?? 'User') }}">
                    <div>
                        <p class="fw-medium mb-0">{{ auth()->user()->name ?? 'User' }}</p>
                        <p class="small mb-0">
                            @if(auth()->user()->type == 'admin')
                                 Admin
                            @elseif(auth()->user()->type == 'cheff')
...
@endphp
                    ...
                    <div class="dropdown">
                        <button class="btn neo-btn dropdown-toggle d-flex align-items-center" type="button" 
                            ...
                            <img src="{{ auth()->user()->avatar ?? 'https://i.pravatar.cc/32' }}" 
                            ...
                            <li class="dropdown-header">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="small text-muted">{{ auth()->user()->email ?? 'user@example.com' }}</div>
